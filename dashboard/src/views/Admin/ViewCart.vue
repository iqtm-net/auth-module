        ...
        <table border="0" class="table cust-table uk-card uk-card-default">
            <thead>
                <tr style="">
                    ...
                    <th style="width:200px;">sender address</th>
                    <th style="width:200px;">receiver name</th>
                    <th style="width:200px;">receiver phone</th>
                    <th style="width:200px;">receiver address</th>
            ...
            <tbody v-for="(order) in orders.data" :key="order.id">
                <tr>
                    ...
                    <td style="width:200px;" class="text-center">
                        <router-link :to="`/in_cart_order/${order.account_type}/${order.id}`">
                            <button class="btn btn-outline-info"><span class="fa fa-eye"></span></button>
                        </router-link>
                        <!--<router-link v-if="admin" :to="`/order_update/${order.id}`">
                            <button class="btn btn-outline-success"> <span class="fa fa-pencil"></span> </button>
                        </router-link>-->
                    </td>
                    <td style="width:200px;">{{ order.first_name }}{{ order.last_name }}</td>
                    <td style="width:200px;">{{ order.sender_phone_number }}</td>
                    <td style="width:200px;">{{ order.address_state }}</td>
    ...
    export default {
        ...
        methods: {
            fetchArticles(){
                ...
                this.axios.get(process.env.VUE_APP_URL+`/api/Admin/Account_Cart/${this.$route.params.type}/${this.$route.params.user_id}`)
                  .then(res => {
                    this.isLoaded = true;
